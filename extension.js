...
function alignCursors() {
  ...
  const spaceInserts = createInserts(alignBlocks, targetStartChar, targetLength, textEditor.options);
...
module.exports = {
  activate(context) {
    // NOTE: we use registerCommand instead of registerTextEditorCommand because we
    // need greater control over the TextEditorEdit
    context.subscriptions.push(vscode.commands.registerCommand('huntertran.cursor-align.alignCursors', alignCursors));
  },
...
function createInserts(alignBlocks, targetStartChar, targetLength, textEditorOptions) {
  let tabCharacter = vscode.workspace.getConfiguration('align-cursor').get('alignCharacter');

  // default is 4
  let tabSize = textEditorOptions.tabSize || 4;

  switch (tabCharacter) {
    case 'tab': {
      return createTabInsertsFromAlignBlocks(alignBlocks, targetStartChar, targetLength, tabSize);
    }
    case 'space': {
      return createSpaceInsertsFromAlignBlocks(alignBlocks, targetStartChar, targetLength);
    }
    default: {
      // Check if spaces or tabs are being used for indentation
      const insertSpaces = textEditorOptions.insertSpaces;
      const isUsingSpaces = insertSpaces || false;
 ...
 * @param {number}   targetLength    Length to align the blocks to.
 */
function createTabInsertsFromAlignBlocks(alignBlocks, targetStartChar, targetLength, tabSize) {
  let extraTabRequired = targetStartChar % tabSize !== 0;
  ...
  for (let i = 0; i < alignBlocks.length; ++i) {
    ...
    if (startDist > 0 || extraTabRequired) {
      ...
      tabInserts.push(createTabInsert(alignBlock.line, alignBlock.startChar, startDist, tabSize, extraTabRequired));
    }
    if (endDist > 0) {
      // insert tabs after the align block to align the right side
      tabInserts.push(createTabInsert(alignBlock.line, alignBlock.endChar, endDist, tabSize, extraTabRequired));
    }
  }
 ...
 * @returns Tab insert.
 */
function createTabInsert(line, startChar, dist, tabSize, extraTabRequired = false) {
  let tabCount = Math.ceil(dist / tabSize);

  if (extraTabRequired && startChar % tabSize !== 0) {
    tabCount++;
  }
